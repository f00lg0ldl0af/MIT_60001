...
@patch('ps3.input', create=True)
def test_play_hand(self, mocked_input):
    """
    ...
    # dictionary of words and scores WITH wildcards
    words = {"jar": 90, "f*x": 306, "!!": 306}
    
    # test 1
    hand = {'a': 1, 'j': 1, 'e': 1, 'f': 1, 'r': 1, 'x': 1, '*': 1}
    
    # if input "jar", should return 90 points & hand: * f x e 
    mocked_input.side_effect = ["jar", "f*x", "!!"] # an iterable
    result = play_hand(hand, word_list)
    if result != 306:
        print("FAILURE: test_play_hand()")
        print("\tExpected", words['!!'], "points but got " + \
              str(result))
        failure = True      
